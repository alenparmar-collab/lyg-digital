  ...
  return (
    <Shell name={user.name}>
      ...
      <p className={styles.privacy}>
        Member details are held so the committee can contact members about LYG, and are not
        shared outside the committee.
      </p>
      ...
      <form className={styles.search} method="get" role="search">
        ...
        {query ? (
          <Link className={styles.clear} href="/committee/registrations">
            Clear
          </Link>
        ) : null}
      </form>
...
function Shell({ name, children }: { name: string; children: React.ReactNode }) {
  return (
    <main className={styles.page}>
      <CommitteeHeader name={name} back={{ href: "/committee", label: "Back to the summary" }} />
      {children}
    </main>
  );
}
